...
6. **Retry exhaustion UX** – After max retries, only a single "Retry" button is shown. Could add exponential backoff or auto-retry with a countdown.

7. **Search remove/clear control missing** – There is no dedicated button to quickly clear the current search term, which can slow down filtering workflows on mobile and desktop.
